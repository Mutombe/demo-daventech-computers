...
function HeroSection() {
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen overflow-hidden bg-neutral-950">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center min-h-[70vh]">
          <motion.div style={{ y: textY, opacity }}>
            <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1, delay: 0.3 }}
              className="w-16 h-[2px] bg-gradient-to-r from-cyan-500 to-cyan-400/50 mb-6 origin-left" />
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}
              className="text-cyan-400 text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-8" style={{ fontFamily: 'var(--font-sans)' }}>{hero.badge}</motion.p>
            ...
            <div className="overflow-hidden">
              {(hero.titleParts || []).map((part, i) => (
                <motion.div key={i} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
                  <h1 className={`font-heading leading-[0.92] tracking-tight ${part.highlight ? 'bg-gradient-to-r from-cyan-500 via-cyan-400 to-yellow-500 bg-clip-text text-transparent' : 'text-white'}`}
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.1 }}
              className="flex items-center gap-3 mt-6 border border-cyan-500/20 bg-cyan-500/5 px-4 py-2 w-fit">
              <ShieldCheck size={20} weight="fill" className="text-cyan-500" />
              <span className="text-cyan-400/80 text-xs uppercase tracking-[0.15em] font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>Certified Repair Centre</span>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.3 }} className="flex flex-wrap gap-4 mt-10">
              <Link to="/contact" className="group inline-flex items-center gap-3 bg-cyan-500 text-neutral-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-cyan-400 hover:shadow-xl hover:shadow-cyan-500/20 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
                {hero.ctaPrimary} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to="/projects" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-cyan-500/50 hover:text-cyan-400 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
                {hero.ctaSecondary}
              </Link>
            </motion.div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.2, delay: 0.4 }} className="relative hidden lg:block">
            <div className="relative">
              ...
              <div className="absolute -top-3 -right-3 w-full h-full border-2 border-cyan-500/20 -z-10" />
              <div className="absolute bottom-6 left-6 bg-neutral-950/90 border border-cyan-500/20 px-5 py-3 backdrop-blur-sm">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white/60 text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Fast Turnaround</span>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }}
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2 }}><ArrowDown size={14} className="text-cyan-500/40" /></motion.div>
      </motion.div>
    </section>
  );
}
...
function TrustStrip() {
  ...
  const inView = useInView(ref, { once: true, margin: '-50px' });
  return (
    <section ref={ref} className="bg-neutral-900 border-y border-cyan-500/10 py-10 sm:py-14">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {siteData.stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.1 }} className="text-center">
              <div className="font-heading text-cyan-500 leading-none" style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)' }}>
    ...
    <section ref={ref} className="bg-neutral-950 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-14 sm:mb-20">
          ...
          <h2 className="font-heading text-white leading-[0.92]" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>Security <span className="text-cyan-500">Packages</span></h2>
        </motion.div>
        <div className="grid md:grid-cols-3 gap-4 sm:gap-6">
          {tiers.map((tier, i) => (
            <motion.div key={tier.name} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.12 }}
              className={`relative p-8 border ${tier.highlight ? 'border-cyan-500/40 bg-cyan-500/5' : 'border-white/5 bg-neutral-900/50'}`}>
              {tier.highlight && <div className="absolute top-0 left-0 right-0 h-[2px] bg-cyan-500" />}
              {tier.highlight && <div className="absolute -top-3 right-6"><span className="bg-cyan-500 text-neutral-950 text-[10px] uppercase tracking-[0.2em] px-3 py-1 font-bold" style={{ fontFamily: 'var(--font-sans)' }}>Popular</span></div>}
              ...
              <div className="space-y-3 mb-8">
                {tier.features.map(f => (<div key={f} className="flex items-center gap-3"><CheckCircle size={16} weight="fill" className={tier.highlight ? 'text-cyan-500' : 'text-white/20'} /><span className="text-white/60 text-sm" style={{ fontFamily: 'var(--font-sans)' }}>{f}</span></div>))}
              </div>
              <Link to="/contact" className={`group inline-flex items-center gap-2 px-6 py-3 text-xs uppercase tracking-[0.15em] font-semibold transition-all ${tier.highlight ? 'bg-cyan-500 text-neutral-950 hover:bg-cyan-400' : 'border border-white/10 text-white hover:border-cyan-500/40 hover:text-cyan-400'}`} style={{ fontFamily: 'var(--font-sans)' }}>
    ...
    <section ref={ref} className="bg-neutral-900 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          <div className="w-12 h-[2px] bg-cyan-500 mb-6" />
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div><p className="text-cyan-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Expertise</p>
              <h2 className="font-heading text-white leading-[0.92]" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>Security <span className="text-cyan-500">Solutions</span></h2></div>
            <Link to="/services" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-cyan-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>All Services <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" /></Link>
          </div>
        </motion.div>
            ...
            return (
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08*i }} className={i === 0 ? 'sm:col-span-2 lg:col-span-2' : ''}>
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className={`group relative block overflow-hidden ${i === 0 ? 'aspect-[16/9] sm:aspect-[2/1]' : 'aspect-[3/4]'}`}>
                  ...
                  <div className="absolute top-4 right-5 z-10"><span className="text-cyan-500/15 font-heading text-6xl sm:text-7xl leading-none">{String(i+1).padStart(2,'0')}</span></div>
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 bg-cyan-500 flex items-center justify-center group-hover:bg-cyan-400 transition-colors"><Icon size={18} weight="fill" className="text-neutral-950" /></div>
                  <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 z-10">
                    ...
                    <div className="flex items-center gap-2 mt-3 text-cyan-500 group-hover:translate-x-1 transition-transform"><span className="text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Explore</span><ArrowRight size={14} /></div>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-cyan-500 to-cyan-400 z-10" />
                </Link>
              </motion.div>
            );
          })}
    ...
    <section ref={containerRef} className="bg-neutral-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
          <div><div className="w-12 h-[2px] bg-cyan-500 mb-6" /><p className="text-cyan-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Track Record</p>
            <h2 className="font-heading text-white leading-[0.92]" style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)' }}>Our <span className="text-cyan-500">Portfolio</span></h2></div>
          <Link to="/projects" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-cyan-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>View All <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" /></Link>
        </motion.div>
      </div>
          ...
          {projects.items.map(project => (
            <div key={project.slug} className="group relative flex-shrink-0 w-[300px] sm:w-[360px] lg:w-[420px] overflow-hidden">
              <div className="aspect-[3/4] overflow-hidden relative">
                ...
                <div className="absolute top-5 left-5 z-10"><span className="bg-cyan-500 text-neutral-950 text-[10px] uppercase tracking-[0.2em] px-3 py-1.5 font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>{project.category}</span></div>
              </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            ...
            <h2 className="font-heading text-white leading-[0.95] mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The {business.name}<br /><span className="text-cyan-500">Advantage</span>
            </h2>
            ...
            <div className="flex gap-10 sm:gap-16">
              {[{ v: business.established, l: 'Founded' }, { v: business.projectsCompleted, l: 'Fixed' }, { v: '24/7', l: 'Operations' }].map(s => (
                <div key={s.l}><div className="text-cyan-500 font-heading text-3xl sm:text-4xl leading-none">{s.v}</div><div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>{s.l}</div></div>
              ))}
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            ...
            <div className="absolute -bottom-8 -left-6 w-[45%] overflow-hidden border-4 border-neutral-950 shadow-2xl"><img src="https://images.unsplash.com/photo-1558002038-1055907df827?w=600&q=80" alt="Systems" className="w-full aspect-square object-cover object-center" loading="lazy" /></div>
            <div className="absolute -top-4 -right-4 sm:-right-6 bg-cyan-500 text-neutral-950 p-5 sm:p-7 shadow-2xl"><div className="text-center"><div className="text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Est.</div><div className="font-heading text-3xl sm:text-4xl leading-none mt-1">{business.established}</div></div></div>
...
function TestimonialsSection() {
  ...
  return (
    <section ref={ref} className="relative bg-neutral-900 py-24 sm:py-32 lg:py-40 overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                {t.avatar && <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover border-2 border-cyan-500/30" loading="lazy" />}
                ...
                <div className="flex gap-0.5 mt-1">{[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-cyan-500" />)}</div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-cyan-500 transition-colors" aria-label="Previous"><CaretLeft size={16} /></button>
            <div className="flex gap-2">{homeTestimonials.map((_, i) => (<button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-cyan-500' : 'w-3 bg-white/10'}`} aria-label={`Testimonial ${i+1}`} />))}</div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-cyan-500 transition-colors" aria-label="Next"><CaretRight size={16} /></button>
          </div>
        </motion.div>
      </div>
    </section>
    ...
    <section ref={ref} className="relative py-24 sm:py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.9 }}>
          <div className="w-12 h-[2px] bg-cyan-500 mx-auto mb-6" />
          <h2 className="font-heading text-white leading-[0.95] mb-6" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>Ready for <span className="text-cyan-500">Fast Repairs?</span></h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/contact" className="group inline-flex items-center gap-3 bg-cyan-500 text-neutral-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-cyan-400 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
              Free Diagnosis <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </Link>
